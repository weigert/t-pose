...
int main( int argc, char* args[] ) {
	...
	triangle2D.bind<ivec4>("index", tpose::trianglebuf);
	triangle2D.bind<ivec4>("colacc", tpose::tcolaccbuf);
	...
	tpose::triangulation trA, trWA;
	tpose::triangulation trB, trWB;
	...
	tpose::io::read(&trA, args[1]);
	tpose::io::read(&trWA, args[2]);
	tpose::io::read(&trB, args[3]);
	tpose::io::read(&trWB, args[4]);
	...
	Tiny::view.interface = [&](){
		...
		ImGui::DragInt("Grow", &grow, 1, 0, 10);
			...
			if(ImGui::Button("Compute F")){
				...
				const mat3 T = mat3(
					0.5f/tpose::RATIO, 0.0, 1.0f,
					0.0, -0.5f/tpose::RATIO, 1.0f/tpose::RATIO,
					0, 0, 1
				);
				...
				if( matchX.size() < 8 ){
					...
					cout<<"F_Sampson: "<<tpose::mview::F_Sampson(matchX, matchY)<<endl;
					cout<<"F_LMEDS: "<<tpose::mview::F_LMEDS(matchX, matchY)<<endl;
					cout<<"F_RANSACL "<<tpose::mview::F_RANSAC(matchX, matchY)<<endl;

					// Create output triangulation from the matches???

					tpose::triangulation pruneA, pruneWA;
					tpose::triangulation pruneB, pruneWB;
	...
	Tiny::event.handler = [&](){
		...
		if(!Tiny::event.press.empty() && Tiny::event.press.back() == SDLK_d){
			...
			if(tpose::intriangle(p, tr->triangles[i], tr->originpoints, tr->points, (showA)?warp:1-warp)){
	...
	Tiny::view.pipeline = [&](){
		...
		triangle2D.use();
		triangle2D.uniform("RATIO", tpose::RATIO);
		...
		if(showA){
			...
			tpose::trianglebuf->fill(trA.triangles);
			tpose::tcolaccbuf->fill(trA.colors);
			...
			tpose::trianglebuf->fill(trB.triangles);
			tpose::tcolaccbuf->fill(trB.colors);
