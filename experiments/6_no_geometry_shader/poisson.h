...
namespace sample{
  ...
  void reflect(std::vector<glm::vec2>& set, glm::vec2 a, glm::vec2 b){
    const size_t size = set.size();
    const glm::vec2 dim = b-a;

    for(size_t i = 0; i < size; i++)
      set.push_back(set[i] + glm::vec2( dim.x, 0));
    for(size_t i = 0; i < size; i++)
      set.push_back(set[i] + glm::vec2(-dim.x, 0));
    for(size_t i = 0; i < size; i++)
      set.push_back(set[i] + glm::vec2(0,  dim.y));
    for(size_t i = 0; i < size; i++)
      set.push_back(set[i] + glm::vec2(0, -dim.y));


    for(size_t i = 0; i < size; i++)
      set.push_back(set[i] + glm::vec2( dim.x,  dim.y));
    for(size_t i = 0; i < size; i++)
      set.push_back(set[i] + glm::vec2(-dim.x,  dim.y));
    for(size_t i = 0; i < size; i++)
      set.push_back(set[i] + glm::vec2( dim.x, -dim.y));
