...
int main( int argc, char* args[] ) {
	...
	if(!pio::loader::getframes(frames, {"d.png", "rgb.png"}))
		exit(0);
	...
	const mat3 T = transpose(mat3(
		1.0f / tri::RATIO * 0.5f * 960.0f, 0, 0.5f * 960.0f,
  ...
  particles.SIZE = pointset.size();
	...
	for(size_t t = 0; t < trA.triangles.size(); t++){
		...
		for(size_t i = 0; i < pixelset.size(); i++){
			...
			if(!intriangle(TB*vec3(pixelset[i], 1.0f), trA.triangles[t], trA.points)) continue;
	...
	//	npixels.push_back(p2);
		...
		ncolors.push_back((vec4)tri::col[t]/255.0f);
		ncolors.back().w = 1.0f;
